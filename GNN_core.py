...
class seq_kernel():
    def __init__(self, seq_len, sigma, device):
        self.weight_mean = torch.zeros(seq_len).to(device)
        self.weight_sigma = (torch.ones_like(self.weight_mean) * sigma).to(device)
        self.current_weight = None
        self.device = device
    ...
    def update(self, sample, objective_func):
        ...
        # breakpoint()
        objective_func = objective_func.to(self.device)
        sample = torch.stack(sample, 0) # Ns x k
        ...
        weight_sigma = (weight_sigma * objective_func.unsqueeze(-1)).sum(0)
        self.weight_sigma = (weight_sigma / objective_func.sum()).sqrt()
        self.weight_mean[self.weight_mean > 1] = 1
        self.weight_mean[self.weight_mean < 0] = 0
...
def train(model,train_loader, optimizer,criterion,device,drop_ratio=None,
          ...
          lam=None, lam2=None, update_weight=None, clustering=None):
    # breakpoint()
    model.train()
    ...
    for data in train_loader:  # Iterate in batches over the training dataset.
        ...
        if data_weight is None:
            ...
            addsum = torch.zeros_like(data2.graph_index)
            addsum[1:] = torch.cumsum(torch.bincount(data2.batch), dim=0)[:-1]
            # try:
            weight_index = torch.from_numpy(clustering[data2.graph_index - addsum])
            # except:
            #     breakpoint()
            data_weight_bs = data_weight(weight_index.to(device))
            # data_weight_bs = data_weight_bs.exp() / data_weight.weight.exp().sum().detach()
            data_weight_bs = data_weight_bs.sigmoid()
            try:
                loss1 = criterion(out, data.y.to(device)).mean()
            except:
                breakpoint()
            loss2 = (criterion(out2, data2.y.to(device)) * data_weight_bs).mean()
            reg_loss = ((data_weight_bs.sigmoid() - 1) ** 2).mean()
            loss = loss1 + loss2 * lam + reg_loss * lam2
        # breakpoint()
        loss.backward()  # Derive gradients.
        # breakpoint()
        if data_weight is not None and update_weight:
...
def train_GNN(model, train_loader,val_loader,test_loader,optimizer,criterion,n_epochs,patience,
              ...
              lam=None, lam2=None, update_weight=False, clustering=None):
    ...
    for epoch in range(0, int(n_epochs)):
        train(model=model,train_loader=train_loader,optimizer=optimizer,criterion=criterion,
              ...
              data_weight_optimizer=data_weight_optimizer, lam=lam, lam2=lam2,
              update_weight=update_weight, clustering=clustering)
...
def calc_dist_mat(seq):
    seq_mat = np.stack([np.array(list(list(seq.values())[i])) for i in range(len(seq))], axis=0)
    not_equal = seq_mat[:, np.newaxis,:] != seq_mat[np.newaxis, :, :]
    not_gap = (seq_mat[:, np.newaxis,:] != '-') & (seq_mat[np.newaxis, :, :] != '-')
    hamm_dist = (not_equal & not_gap).sum(-1)
    hamm_dist = (hamm_dist + 1e-5) / (not_gap.sum(-1) + 1e-6)
    hamm_dist[np.arange(len(hamm_dist)), np.arange(len(hamm_dist))] = 0
    df_hamm_dist = pd.DataFrame.from_dict(dict(zip(seq.keys(), hamm_dist)))
    df_hamm_dist.index = seq.keys()
    return df_hamm_dist

def kernel_hamm_dist(seq, kernel):
    seq_mat = np.stack([np.array(list(list(seq.values())[i])) for i in range(len(seq))], axis=0)
    not_equal = seq_mat[:, np.newaxis,:] != seq_mat[np.newaxis, :, :]
    not_gap = (seq_mat[:, np.newaxis,:] != '-') & (seq_mat[np.newaxis, :, :] != '-')
    hamm_dist = ((not_equal & not_gap).astype(float) * kernel[np.newaxis, np.newaxis, :]).sum(-1)
    hamm_dist = (hamm_dist + 1e-5) / (not_gap.sum(-1) + 1e-6)
    hamm_dist[np.arange(len(hamm_dist)), np.arange(len(hamm_dist))] = 0
    df_hamm_dist = pd.DataFrame.from_dict(dict(zip(seq.keys(), hamm_dist)))
    df_hamm_dist.index = seq.keys()

...
def remove_edges_by_distance_and_type(x, old_edges, net_mhc, net_peptide, threshold):
    # breakpoint()
    feature = torch.cat([x], dim=-1)
    feature_mhc = net_mhc.forward(feature)
    feature_peptide = net_peptide.forward(feature)
...
def remove_edges_by_distance(x, old_edges, net_mhc, net_peptide, threshold):
    # breakpoint()
    feature = torch.cat([x], dim=-1)
    feature_mhc = net_mhc.forward(feature)
    feature_peptide = net_peptide.forward(feature)
...
def add_new_edges(E, A, threshold, item_type):
    ...
    return updated_E
def add_edges_by_distance_and_type(x, old_edges, net_mhc, net_peptide, threshold):
    # breakpoint()
    # breakpoint()
    feature = torch.cat([x], dim=-1)
    feature_mhc = net_mhc.forward(feature)

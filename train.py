...
parser.add_argument('--loss_function', type=str, required=False, default='ce')
...
if args.prelin_type == 'linear':
    prelinear_mhc = GNN_core.preLin(input_dim=num_node_features, output_dim=meta_feature_dim,
                                    init_sigma=args.initial_sigma, init_mu=initial_mu, device=device)
    prelinear_peptide = GNN_core.preLin(input_dim=num_node_features, output_dim=meta_feature_dim,
                                        init_sigma=args.initial_sigma, init_mu=initial_mu, device=device)
else:
    prelinear_mhc = GNN_core.prelinear_twolayers(input_dim=num_node_features, output_dim=meta_feature_dim,
                                                 init_sigma=args.initial_sigma, init_mu=initial_mu, device=device)
    prelinear_peptide = GNN_core.prelinear_twolayers(input_dim=num_node_features, output_dim=meta_feature_dim,
                                                     init_sigma=args.initial_sigma, init_mu=initial_mu, device=device)
...
def train_onePoint(current_train_data, current_val_data, current_train_data2, data_weight,
    ...
    if args.loss_function == 'ce':
        ...
        criterion = FocalLoss(alpha=0.2, gamma=2.0, reduction='none').to(device)
